...
class CreatePayrollView(APIView):
    def post(self,request,format='json'):
        ...
        if serializers.is_valid():
            ...
            if calculated_payroll.get("net_pay", 0) > 0:
                serializer_calculated = PayrollSerializer(data=calculated_payroll)
            ...
            return Response(serializer_calculated.errors, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({"message": "Net pay could not be calculated"}, status=status.HTTP_400_BAD_REQUEST)
